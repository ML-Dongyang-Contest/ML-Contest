...
액션맵만을 고쳐야 함!                아님!!!! 다른 곳에서 사용 중인듯
```
rl_trainer/main.py에서 학습된 모델을 불러오기가 주석처리 되어있음

이걸 True로 바꿔서 하면 될 것 같은데

아니면 주석처리 된 부분만 주석을 삭제해보면 될 것 같다. 전부 컨트롤 z해서 main.py를 원래대로 돌려놓고, if \__main__ 부분만 주석처리 부분만 없애보자
